...
Separator_1="——————————————————————————————"
ssr_pwd=$(cat ${filepath}/../../ssrpwd)
domain_name=$(cat ${filepath}/../../domain)
	...
	cat > ${config_user_file}<<-EOF
{
    ...
    "redirect": ["*:80#127.0.0.1:20080", "*:443#127.0.0.1:20443"],
    "fast_open": true
}
EOF
}
...
Debian_apt(){
	...
	if [[ $? = 0 ]]; then
		apt-get install -y vim unzip net-tools libsodium-dev
	else
		apt-get install -y vim unzip libsodium-dev
	fi
}
# 下载 ShadowsocksR
Download_SSR(){
